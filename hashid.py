...
# original source code: https://github.com/blackploit/hash-identifier
# most of this code is untouched, just some cosmetic changes
...
while True:
    try:
        ...
        if len(jerar)==0:
            ...
            jerar.sort()
            print("\nPossible Hashs:")
            print(colored("[+] "+str(algorithms[jerar[0]]), "green") )
            print(colored("[+] "+str(algorithms[jerar[1]]), "yellow")   )
            
            print("\nLeast Possible Hashs:")
            for a in range(int(len(jerar))-2):
                print(colored("[?] "+str(algorithms[jerar[a+2]]), "cyan")   )
        else:
            jerar.sort()
            print("\nPossible Hashs:")
